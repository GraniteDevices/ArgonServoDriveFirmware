...
//by tero
#ifndef PROGRAM_FLASH_OFFSET
#define PROGRAM_FLASH_OFFSET 0
#warning PROGRAM_FLASH_OFFSET not defined! Must be specified at compiler parameteres like -DPROGRAM_FLASH_OFFSET=0
#endif

#define VECT_TAB_OFFSET  (0x00+PROGRAM_FLASH_OFFSET) /*!< Vector Table base offset field.
...
/*choose between internal RC and crystal */
//#define USE_INTERNAL_RC_OSC
...
/** @addtogroup STM32F2xx_System_Private_Functions
  * @{
  */
//NOPdelay(2000)=100us @ 120MHz
void StartUpDelay( unsigned long dly )
{
	unsigned long i;
  	for( i = 0; i < dly; i++ )
  		if (dly == i)
...
{
  StartUpDelay(200000); //dummy wait for voltage rise before setting CPU freq to max
...
/******************************************************************************/
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    RCC->PLLCFGR = (PLL_M_HSI) | (PLL_N << 6) | (((PLL_P >> 1) -1) << 16) |
...
/******************************************************************************/
  ...
  if (HSEStatus == (uint32_t)0x01)
  {
    ...
    /* Configure Flash prefetch, Instruction cache, Data cache and wait state */
    //prefect enabled -> more noisy ADC according to app notes especially near max input voltage
    //FLASH->ACR = FLASH_ACR_PRFTEN | FLASH_ACR_ICEN | FLASH_ACR_DCEN | FLASH_ACR_LATENCY_3WS;
     //disabled prefetch -> lower noise on ADC, see ST AN4073. in our tests no much difference found
